...
from transpilex.config.base import YII_COOKIECUTTER_REPO, YII_ASSETS_PRESERVE
...
class BaseYiiConverter:
    ...
    def init_create_project(self):
        try:
            ...
            Log.success("Yii project created successfully")

        except:
            Log.error("Yii project creation failed")
            return

        files = find_files_with_extension(self.config.pages_path)
        copy_and_change_extension(files, self.config.pages_path, self.project_templates_path, self.config.file_extension)
    ...
    def _replace_all_includes(self, content: str):
        ...
        for frag in fragments:
            ...
            if params:
                ...
                replacement = f"<?php echo $this->render('{clean_path}', array({param_str})) ?>"
            else:
                replacement = f"<?php echo $this->render('{clean_path}'); ?>"
...
class YiiGulpConverter(BaseYiiConverter):
    ...
    def create_project(self):
        ...
        if self.config.asset_paths:
            copy_assets(self.config.asset_paths, self.config.project_assets_path, preserve=YII_ASSETS_PRESERVE)
